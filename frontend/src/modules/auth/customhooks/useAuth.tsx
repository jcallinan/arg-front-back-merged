...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
   ...
   const { recordMeApiCall, clearTimings } = useTokenTiming();
   ...
   const extractUserData = (responseData: AuthApiResponse): User => {
      ...
      const userDetails = responseData.user || responseData.userDetails || responseData;
      
   

      const extractedUser = {
         id: userDetails?.UserId || userDetails?.userId || "unknown",
         email: userDetails?.email || userDetails?.Email || "unknown@example.com",
         name: userDetails?.userDisplayName || userDetails?.displayName || userDetails?.name || "User",
   ...
   const handleSuccessfulLogin = async (): Promise<void> => {
      ...
      setError(null);
         ...
         if (data.items && typeof data.items === 'object' && 'iv' in data.items && 'payload' in data.items) {
         ...
         // Save the complete response for future reference
         setLastLoginResponse(data);
         
         // Record the successful getUserDetails API call for token timing
         recordMeApiCall();
...
export const useAuth = (): AuthContextType => {
   ...
   if (context === undefined) {
      ...
      if (process.env.NODE_ENV === 'development') {
         console.error("🔧 This might be caused by hot module reloading. Try refreshing the page.");
      }
