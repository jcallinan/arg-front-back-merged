   ...
   const handleSubmit = async () => {
      ...
      try {
         ...
         const processPayload = checkData.map((check) => {
            ...
            try {
               if (check.clearDate.includes("/")) {
                  // Convert MM/DD/YY to MMDDYY format
                  const [month, day, year] = check.clearDate.split("/");
                  ...
                  apiDateFormat =
                     formatNumberToMMDDYY(dateObj.toISOString()) ||
                     check.clearDate;
               }
            } catch (error) {
               console.warn(
                  `Failed to convert date ${check.clearDate} for check ${check.checkNumber}:`,
                  error
               );
