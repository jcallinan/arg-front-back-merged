...
const VendorFileMaintenance1099: React.FC = () => {
   ...
   const handleApiError = (error: any, context: string) => {
      // Handle the specific error structure from the API response
      // Structure: Response { error: { error: { code: "NOT_FOUND", message: "...", details: [...] } } }
      const errorData = error?.error?.error || error?.response?.data?.error || error?.error || error;
      
      let title = "Error";
      let subtitle = `An error occurred while ${context}. Please try again.`;
      
      if (errorData?.code === 'NOT_FOUND') {
         // Check if there are specific details about the vendor not found
         const details = errorData?.details || [];
         const vendorDetail = details.find((detail: any) => detail.field === 'vendor');
         
         if (vendorDetail) {
            title = "Vendor Not Found";
            subtitle = vendorDetail.message;
   ...
   const fetchVendorsByYearData = async (
      ...
      try {
         ...
         if (typeof p.hasNextPage === 'boolean') {
            ...
            typeof p.page === 'number' && typeof p.totalPages === 'number'
         ) {
   ...
   const handlePaginationChange = (page: number, size: number) => {
      ...
      if (page >= totalPagesAvailable && shouldLoadMore) {
         const nextApiPage = lastApiPage > 0 ? lastApiPage + 1 : Math.floor(allVendorData.length / 500) + 1;
   ...
   const fetchVendorDetailsByYearData = async (vendorNo: number) => {
      try {
         ...
         // Handle specific NOT_FOUND error structure
         handleApiError(error, `fetching vendor details for vendor #${vendorNo}`);
         return null;
      }
   };
   ...
   const handleViewClick = async (record: Vendor1099Data) => {
      ...
      const vendorDetailsByYear = await fetchVendorDetailsByYearData(record.vendorNo);
   ...
   const handleEditClick = async (record: Vendor1099Data) => {
      try {
         ...
         // If the new API returns data, use it; otherwise fallback to the original API
         const vendorDetails = vendorDetailsByYear || await fetchVendorDetailsData(record.vendorNo);
   ...
   const handleCustomSave = async (vendorData: any) => {
      try {
         const updatePayload: UpdateVendorByYearData = {
            ...
            vendorAdpPayrollId: vendorData.vendorAdpPayrollId,
      ...
      <div>
         {isEditMode && selectedVendorForEdit ? (
            <AddVendor
               ...
               onSaveSuccess={() => { /* handled by local success modal */ }}
