...
export const useLmsEntries = (companyNo: number = 10) => {
  const api = useApi();
...
export const useLmsVoucherSummary = (companyNo: number = 10) => {
  const api = useApi();
...
export const useLmsBatchCreate = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.removeQueries({ queryKey: ["lmsCarrierInvoices"] });
