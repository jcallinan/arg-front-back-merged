...
export const usePaperEntries = (companyNo: number = 10) => {
  const api = useApi();
...
export const usePaperVoucherSummary = (companyNo: number = 10) => {
  const api = useApi();
...
export const usePaperBatchCreate = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.removeQueries({ queryKey: ["carrierInvoices"] });
