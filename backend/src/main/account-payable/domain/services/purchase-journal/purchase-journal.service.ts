...
export class PurchaseJournalService {
  ...
  async submitPurchaseJournal(dto: SubmitPurchaseJournalDto) {
    ...
    const normalEntries = entries.filter(entry => entry.prepaidCode !==  MakePrepaidFlag.PREPAID);
    const prepaidEntries = entries.filter(entry => entry.prepaidCode ===  MakePrepaidFlag.PREPAID);
