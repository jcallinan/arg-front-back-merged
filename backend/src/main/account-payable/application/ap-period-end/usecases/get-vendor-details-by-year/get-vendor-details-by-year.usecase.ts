...
import { VendorDto } from "../../../vendor-management/dto/vendor-management.dto";
import { VendorInterface } from "@src/main/account-payable/domain/interface/vendor.interface";
import { vendorDetailByYearDto } from "../../dto/ap-period-end.dto";
...
@Injectable()
export class GetVendorDetailsByYearUsecase {
    private readonly logger = new AppLogger(GetVendorDetailsByYearUsecase.name);
    ...
    async execute(data: vendorDetailByYearDto): Promise<VendorDto | null> {

        const { vendorCompanyNumber, vendorNo, year } = data

        this.logger.log(`Get vendor details for Year: ${year}`);

        // Interface Call
        const vendorDetails = await this.vendorInterface.getVendorDetailsByYear(vendorCompanyNumber, JSON.parse(vendorNo), year);
        ...
        const vendorWithDescriptions = await this.fetchVendorDescriptions(vendorDetails);
        
        
        return {
           ...vendorWithDescriptions
        };
    }
