  ...
  async cacheDataForCompany(@Body() body: { companyNo: number }) {
    ...
    const result = {
      ...
      summary: {
        ...
        message: `Successfully cached ${companyResult.cachedCompanies} companies, ${vendorResult.cachedVendors} vendors, ${apdateResult.cachedApdates} APDATE records, ${glMasterResult.cachedGlMasters} GL Master records, ${gstableResult.cachedGSTables} GSTable records, and ${freightInvoiceResult.cachedFreightInvoices} FreightInvoice records for company ${body.companyNo}`,
      },
    };
