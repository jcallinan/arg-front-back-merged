...
import { convertDateFormat, convertMMDDYYtoYYYYMMDD, DATE_FORMATS } from "@src/shared/utils/format-date";
...
export function mapToAptranHeaderInsertDto(params: {
  ...
  return VoucherHeader.create({
    ...
    dueDate,
    singleCheck: "",
    holdCode: source.holdCode || "",
    holdDesc: (source.holdDescription || "").substring(0, 25),
    ...
    retentionGl,
    retentionPct: 0,
    prepaidCheckdate: voucherType === VoucherType.PAID ? convertDateFormat(source.paidOnYymmdd, DATE_FORMATS.YYMMDD, DATE_FORMATS.MMDDYY_MMDDYY) : 0,
    totalFreight: (source.freightTotal || 0) * -1,
    salesOrderNo: source.salesOrderNo || 0,
    srn: source.salesSrnNo || 0,
    carrierId: (
      vendorData?.vendorCarrierId ||
      source.carrierId ||
      ""
    ).substring(0, 6),
