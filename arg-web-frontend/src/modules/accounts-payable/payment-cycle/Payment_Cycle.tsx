...
const Payment_Cycle = () => {
   ...
   useEffect(() => {
      const useCaseMap: Record<string, string> = {
         ...
         // For Employee Expense and Utility, follow same as Wire when applicable
         "Employee Expense": "Payment_Cycle_Wire_Submit",
         Utility: "Payment_Cycle_Wire_Submit",
      };
   ...
   const handleButtonClick = async () => {
      ...
      // Step 0: Payment Selection -> when voucher type is Check/ACH/Wire/Employee Expense/Utility, call POST generate report
      if (
         currentStep === 0 &&
         ["Check", "ACH", "Wire", "Employee Expense", "Utility"].includes(
            voucherType
         )
      ) {
      ...
      if (currentStep === 1 && ["Check", "ACH", "Wire", "Employee Expense", "Utility"].includes(voucherType)) {
         try {
            const useCaseMap: Record<string, string> = {
            ...
            const useCase = useCaseMap[voucherType as keyof typeof useCaseMap];
            await generateReport(useCase, []);

            if (["ACH", "Wire", "Employee Expense", "Utility"].includes(voucherType)) {
               // Mark final API as successful so the success modal/open flow works.
               // Intentionally do NOT show a success toaster here; the success modal is sufficient.
