...
const VoucherMaintenance: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // When suppressLoadedToast is true (e.g. right after a void), skip the
    // automatic "Loaded X voucher records" toast entirely. The caller may
    // choose to show a delayed toast instead.
    ...
    const baseColumns: (ColumnType<any> & { filterable?: boolean })[] = [
      {
        ...
        render: (_, record) => (
          <div className="action-icons flex-all flex-align">
            <ActionPermissionGuard actionId="voucher-maintenance.search" requireManage={false}>
            ...
            {(activeVoucherType === "UNPAID" ||
              ...
              <ActionPermissionGuard actionId="voucher-maintenance.modify-status">
            ...
            {(activeVoucherType === "UNPAID" ||
              (activeVoucherType === "ALL" && record.status === "Unpaid")) && (
              <ActionPermissionGuard actionId="voucher-maintenance.modify-discount">
            ...
            {record.status !== "Cancelled" && (
              <ActionPermissionGuard actionId="voucher-maintenance.cancel">
                <Tooltip
                  title={
                    record.status === "Paid" ? "Void Voucher" : "Cancel Voucher"
                  }
                >
                  <img
                    className="action-icon"
            ...
            <div className="cards-container">
              ...
              <div className="single-row">
                ...
                <Card
                  ...
                  value={formatMMDDYYForDisplay(summaryData.lastPaidDate)}
                />
              </div>
            </div>
          </div>
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            setTimeout(async () => {
              ...
              setAllVoucherData([]);
              // Prevent automatic "Loaded X voucher records" toast
              ...
              const loadedCount = voucherResult.data?.length ?? 0;
              if (voucherResult.data) {
                setAllVoucherData(voucherResult.data);
              }
              if (loadedCount > 0) {
                setTimeout(() => {
                  // Re-enable automatic loaded toast behaviour for future queries
                  setSuppressLoadedToast(false);
                  showToasterMessage(
                    "success",
                    "Success",
                    `Loaded ${loadedCount} voucher records`
                  );
                }, 2000);
              }
            }, 100);
