...
const SogasProcess: React.FC<SogasProcessProps> = ({
  ...
  const isRowSelectable = (entry: VoucherEntryUI) => {
     const hasPrepaidCode = entry.prepaidCode && entry.prepaidCode !== "";
     const hasPrepaidCheckDate = !!entry.prepaidCheckdate;
     const isErrorStatus = entry.status === "error";
     return !(isErrorStatus || (hasPrepaidCode && !hasPrepaidCheckDate));
  };
  ...
  const handleSelectAllRows = () => {
      ...
      title: (
         ...
         render: (_, record) => (
            <Checkbox
               ...
               onChange={() => isRowSelectable(record) && handleRowSelect(record.key)}
         ...
         render: (_, record) => (
            <div className="action-icons flex-all">
               <ActionPermissionGuard
                  actionId="voucher-entry.sogas.view"
                  requireManage={false}
               >
                  <Tooltip title="View">
                     <EyeOutlined
               ...
               <ActionPermissionGuard actionId="voucher-entry.sogas.edit">
                  <Tooltip title="Edit">
                     <EditOutlined
               ...
               <ActionPermissionGuard actionId="voucher-entry.sogas.delete">
   ...
   const handlePostModalSubmit = async () => {
      ...
      const hasPrepaidEntry = selectedEntries.some(
         (entry) => entry.prepaidCode && entry.prepaidCode !== ""
      );
      ...
      <div>
         <div className="flex-between">
            ...
            <div className="action-buttons">
               <ActionPermissionGuard actionId="voucher-entry.sogas.post">
                  <CustomStyledButton
                     name="postToPurchaseJournal"
                     label={<h6>{VOUCHER_ENTRY_TEXTS.postToPurchaseJournal}</h6>}
                     icon={
                        <img src={FileIcon} alt="file" className="file-icons" />
                     }
         ...
         {viewRecord && (
            <ViewVoucherModal
              ...
              editActionId="voucher-entry.sogas.edit"
               onEdit={() => {
                  if (viewRecord) {
                     handleEditClick({
