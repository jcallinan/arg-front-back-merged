...
const ReviewBatch = forwardRef<
   { refreshData: () => void },
   FlexiProcessProps & { uploadStatusData?: UploadStatusData; enabledQueries?: boolean }
>(({ uploadStatusData, enabledQueries = true }, ref) => {
   ...
   } = useLmsEntries(10, enabledQueries);

   const {
      data: voucherSummary,
      refetch: refetchLmsVoucherSummary,
   } = useLmsVoucherSummary(10, enabledQueries);
  ...
  useImperativeHandle(ref, () => ({
     refreshData: () => {
        (async () => {
          await Promise.all([refetchLmsEntries(), refetchLmsVoucherSummary()]);
          // Only after both review batch GETs succeed, invalidate batch-entries (LMS)
          queryClient.invalidateQueries({ queryKey: ["lmsCarrierInvoices"] });
        })();
     },
  }));

   useEffect(() => {
      if (!enabledQueries) return;
   ...
   }, [enabledQueries, location.state, location.pathname, navigate, refetchLmsEntries, refetchLmsVoucherSummary]);

   // On websocket summary, update UI counters but avoid manual refetch;
   // let React Query handle the single fetch on tab enable/mount.
   useEffect(() => {
      if (!enabledQueries) return;
   ...
   }, [enabledQueries, uploadStatusData]);

   // After both entries and summary have completed fetching once following a websocket summary,
   // invalidate the LMS batch-entries list exactly once so it refreshes after review batch data.
   const carrierInvalidateOnceRef = useRef(false);

   // Reset "invalidate once" guard when a new websocket cycle happens
   useEffect(() => {
      carrierInvalidateOnceRef.current = false;
   }, [uploadStatusData?.id, uploadStatusData?.summary?.totalUploads]);

   useEffect(() => {
      if (!enabledQueries) return;
      if (!uploadStatusData?.summary) return;
      // Ensure entries and summary are present (React Query has finished)
      const entriesReady = Array.isArray(lmsEntriesData);
      const summaryReady = !!voucherSummary;
      if (!entriesReady || !summaryReady) return;
      if (carrierInvalidateOnceRef.current) return;

      // Trigger batch-entries refresh now that review batch APIs are done
      queryClient.invalidateQueries({ queryKey: ["lmsCarrierInvoices"] });
      carrierInvalidateOnceRef.current = true;
   }, [enabledQueries, uploadStatusData?.summary, lmsEntriesData, voucherSummary, queryClient]);
   ...
   const isRowSelectable = (entry: LmsEntryItem) => {
      const status = (entry.status || "").toString().toUpperCase();
      const hasErrorStatus = status === "E" || status === "ERROR";
      const prepaidCode = (entry as any).prepaidCode;
      const prepaidCheckdate = (entry as any).prepaidCheckdate;
   ...
   const handlePostToPurchaseModalSubmit = async () => {
      ...
      const hasPrepaidEntry = selectedEntries.some((entry) => {
         const prepaidCode = (entry as any)?.prepaidCode;
         return prepaidCode && prepaidCode !== "";
      });
         ...
         render: (_, record) => (
            <div className="action-icons flex-align">
               <ActionPermissionGuard
                  actionId="voucher-entry.lms.view"
                  requireManage={false}
               >
               ...
               <ActionPermissionGuard actionId="voucher-entry.lms.edit">
               ...
               <ActionPermissionGuard actionId="voucher-entry.lms.delete">
      ...
      <>
         <div className="content-card-body">
            <div className="flex-between">
               ...
               <div className="action-buttons">
                  <ActionPermissionGuard actionId="voucher-entry.lms.post">
                     <CustomStyledButton
         ...
         {viewRecord && (
            <ViewVoucherModal
               ...
               editActionId="voucher-entry.lms.edit"
               onEdit={() =>
                  handleEditClick({
                     entryNo: viewRecord.entryNo,
                     companyNo: viewRecord.companyNo,
