...
import { hasErrorDMessage } from "@utils/apiErrorUtils";
...
const YearEnd1099ProcessMenu: React.FC = () => {
   ...
   const showErrorNotification = (subtitle: string) => {
      setNotification({
         visible: true,
         type: "error",
         title: "Error",
   ...
   const handleTestPrintConfirm = async () => {
      ...
      try {
         ...
         const { hasError, message } = hasErrorDMessage(response);

         if (hasError) {
            showErrorNotification(
               message || "An unexpected error occurred during test print"
            );
            return;
         }
         ...
         showErrorNotification(
            "An unexpected error occurred during test print"
         );
      }
   };
   ...
   const handlePrint = async () => {
      ...
      try {
         ...
         if (hasError) {
            showErrorNotification(
               message || "An unexpected error occurred while printing"
            );
            return;
         }
      ...
      if (currentStep === 0) {
         try {
            ...
            if (hasError) {
               showErrorNotification(
                  message ||
                     "An unexpected error occurred while submitting options"
               );
               return;
            }

            // Treat a successful call (no thrown error) as success; do not show toaster
            if (response && currentStep < stepperTitles.length - 1) {
               setCurrentStep((prev) => prev + 1);
            }
            return;
         } catch (error: any) {
            console.error("Error submitting Select Options:", error);

            const responseData = error?.response?.data;
            let errorMessage =
               (typeof responseData?.message === "string" &&
                  responseData.message) ||
               (typeof responseData?.items?.message === "string" &&
                  responseData.items.message) ||
               (Array.isArray(responseData?.items?.message) &&
                  responseData.items.message[0]) ||
               "An unexpected error occurred while submitting options";
      ...
      if (currentStep === 1) {
         try {
            ...
            if (hasError) {
               showErrorNotification(
                  message ||
                     "An unexpected error occurred while creating 1099 file"
               );
               return;
            }
            ...
            if (!irsFilesResult.success) {
               showErrorNotification(
                  irsFilesResult.message || "Failed to generate IRS tax files"
               );
               return; // stop on failure
            }
            ...
            console.error("Error submitting Create 1099 File:", error);
            ...
            <div className="flex-end gap-16">
               ...
               {currentStep === 2 ? (
                  <>
                     ...
                     />
                     <ActionPermissionGuard actionId="year-end-1099-process.next">
                        <CustomStyledButton
                           name="next"
                           label={<h6>Next</h6>}
                           onClick={handleNext}
