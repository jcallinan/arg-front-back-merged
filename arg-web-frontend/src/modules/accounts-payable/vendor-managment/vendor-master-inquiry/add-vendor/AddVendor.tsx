...
import { useGlMasterValidation } from "@hooks/useEmployeeExpense";
import { useApi } from "@/hooks/useApi";
import { normalizeGlAccountErrorMessage } from "@/utils/apiErrorUtils";
import type {
   VendorDetailsDto,
...
interface AddVendorProps {
   ...
   onSaveSuccess?: () => void;
   onCancel?: () => void;
   onSave?: (vendorData: any, contactDetails: any[], extras?: { thisYearPayments?: number }) => Promise<void>;
...
const AddVendor: React.FC<AddVendorProps> = ({
   ...
   const { mutateAsync: validateGlMasterApi } = useGlMasterValidation();
   ...
   const [hasTouchedExpenseGl, setHasTouchedExpenseGl] = useState(false);
         ...
         setFormData(
            (prev) =>
               ({
                  ...
                  // Use the ALT numeric field when available (MMDDYY from API),
                  // and only fall back to vendorLastPaymentDate if needed.
                  vendorLastPaymentDate:
                     (vendor as any).vendorLastPaymentDateAlt
                        ? formatMMDDYYForDisplay(
                             (vendor as any).vendorLastPaymentDateAlt
                          )
                        : vendor.vendorLastPaymentDate
                        ? formatMMDDYYForDisplay(
                             vendor.vendorLastPaymentDate as unknown as number
                          )
                        : prev.vendorLastPaymentDate,
               } as any)
         );
  ...
  const handleChange = (field: string, value: string | number | boolean) => {
     ...
     // Track user interaction with Expense G/L so we only validate
     // when the user has actually tried to enter something.
     if (field === "vendorExpenseGLSub" && !hasTouchedExpenseGl) {
        setHasTouchedExpenseGl(true);
     }
   ...
   const resolveApiDetailMessage = useCallback((errorObj: any) => {
      const detailSources = [
         errorObj?.details,
      ...
      for (const source of detailSources) {
         if (Array.isArray(source)) {
            const detailWithMessage = source.find(
               (detail: any) => detail?.message,
            );
            if (detailWithMessage?.message) {
   ...
   const validateExpenseGlNumber = useCallback(
      async (rawValue?: string | number) => {
         // Skip validation if the user has never interacted with
         // the Expense G/L field. This keeps it optional and
         // prevents errors when it is left blank.
         if (!hasTouchedExpenseGl) {
            clearFieldError("vendorExpenseGLSub");
            return undefined;
         }
         ...
         if (!/^\d{8}$/.test(trimmed)) {
            const message = "Expense G/L must be an 8-digit number";
            setFieldErrors((prev) => ({
               ...prev,
               vendorExpenseGLSub: message,
            }));
            return message;
         }

         try {
            await validateGlMasterApi({
               companyNo: parseInt(companyNo, 10) || 10,
               glNo: Number(trimmed),
            });
            clearFieldError("vendorExpenseGLSub");
            return undefined;
            ...
            const errorData =
               ...
               error?.error ||
               error;
            const detailMessage = resolveApiDetailMessage(errorData);
            const rawApiMessage =
               detailMessage ||
               errorData?.message ||
               "Expense G/L number not found";
            const apiMessage = normalizeGlAccountErrorMessage(rawApiMessage);
            setFieldErrors((prev) => ({
               ...prev,
               vendorExpenseGLSub: apiMessage,
            }));
            return apiMessage;
         }
      },
   ...
   const handleSave = async () => {
      ...
      const expenseGlError = await validateExpenseGlNumber();
      if (expenseGlError) {
         nextErrors.vendorExpenseGLSub = expenseGlError;
      }
      ...
      try {
        ...
        const vendorData = {
            ...
            vendorNameOverflow: formData.vendorNameOverflow ? "Y" : "N",
            ...
            vendorExpenseGLSub: formData.vendorExpenseGLSub
               ? Number(formData.vendorExpenseGLSub)
               : 0,
         ...
         } as VendorDetailsDto & { vendorIsDeleted: string; vendorAreaCode?: number; IdNo1099?: string; contactDetails?: VendorContactDetailDto[]; vendorNameOverflow?: string };
         ...
         if (onSave) {
            await onSave(
              ...
              context === "ap-period-end"
                ? { thisYearPayments: Number(formData.vendorThisYrYtdPaid || 0) }
                : undefined
            );
         } else {
            ...
            if (successErrorObj) {
               ...
               const detailCandidates = [
                  ...
                  successErrorObj?.error?.details,
                  respAny?.data?.details,
                  respAny?.data?.errors,
               ];
               const details = detailCandidates.find((d) =>
                  Array.isArray(d),
               ) as Array<{ field?: string; message?: string }> | undefined;
               if (Array.isArray(details)) {
                  const fieldNameMapping: Record<string, string> = {
                     "Carrier Id": "vendorCarrierId",
                     apGLNo: "vendorExpenseGLSub",
                     apGlNo: "vendorExpenseGLSub",
                     APGLNO: "vendorExpenseGLSub",
                  };
                  ...
                  for (const d of details) {
                     ...
                     const message = normalizeGlAccountErrorMessage(
                        d?.message || "Invalid value",
                     );
                     next[frontendFieldName] = message;
                  }
               setFieldErrors(next);
         ...
         const details = detailCandidates.find((d) => Array.isArray(d)) as
         ...
         if (Array.isArray(details)) {
            for (const d of details) {
               if (d?.field && d?.field.trim() !== "") {
                  ...
                  const frontendFieldName = fieldNameMapping[d.field] || d.field;
                  fieldSpecificErrors[frontendFieldName] = normalizeGlAccountErrorMessage(
                     d?.message || "Invalid value",
                  );
                  hasFieldSpecificErrors = true;
               } else if (d?.message) {
                  // General error without specific field
                  generalServerError = normalizeGlAccountErrorMessage(d.message);
               }
            }
         ...
         if (generalMessage) {
            generalMessage = normalizeGlAccountErrorMessage(generalMessage);
         }
         ...
         <div className="voucherentry-container">
            ...
            <div className="content-div-container add-vendor-main-container">
               ...
               <ExpenseDetailsForm
                  ...
                  errors={fieldErrors}
                  onValidateExpenseGl={async (value) => {
                     await validateExpenseGlNumber(value);
                  }}
               />
               <BankDetailsSection
                  values={{
