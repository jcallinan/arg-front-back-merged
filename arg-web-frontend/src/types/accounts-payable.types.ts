...
export interface PostToPurchase {
  ...
  prepaidCode?: string;
  purchaseJournalStatus?: "error" | "warning";
  cashDisbursementStatus?: "error" | "warning";
}
...
export interface UpdateVendorByYearData {
  ...
  vendorTelephoneNo?: number;
  vendorAreaCode?: number;
  // Year-to-date payments (This Year Payments) to be updated in AP Period End edit
  vendorThisYrYtdPaid?: number;
  vendorFirstName: string;
  vendorMiddleName: string;
  vendorBusinessLastName: string;
...
export interface AddVendorOwnerProps {
  visible: boolean;
  onCancel: () => void;
  isEditMode?: boolean;
