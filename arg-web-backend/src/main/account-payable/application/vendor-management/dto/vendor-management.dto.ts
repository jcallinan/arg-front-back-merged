...
export class vendorDetailsDto {
    ...
    @ApiProperty({ description: "Vendor Is Active", example: 'A', required: true })
    @IsOptional()
    @IsString()
    @MaxLength(1) 
    ...
    @ApiProperty({ description: "This Year YTD Paid", example: 2000.50, required: false })
    @IsOptional()
    @Type(() => Number)
    @Max(999999999.99) // VNTYDP (DECIMAL 11,2)
