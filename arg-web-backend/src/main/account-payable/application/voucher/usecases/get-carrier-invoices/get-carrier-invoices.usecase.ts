...
@Injectable()
export class GetCarrierInvoicesUseCase {
  private readonly logger = new AppLogger(GetCarrierInvoicesUseCase.name);
  ...
  async execute(dto: GetCarrierInvoicesDto): Promise<PaginatedResponse<CarrierInvoiceResponseDto>> {
    dto.invoiceType = INVOICE_TYPE.P
    this.logger.log("Fetching all carrier invoices");
