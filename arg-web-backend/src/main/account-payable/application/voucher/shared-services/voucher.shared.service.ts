...
export class VoucherSharedService {
  ...
  public buildVoucherHeaderData(body) {
    ...
    const data = {
      ...
      invoiceDesc: safeTruncate(invoiceDesc || "", 25) || "",
      ...
      extendedDiscountDueDate:
        (!discountDueDate || discountDueDate === 0)
          ? 0
          : convertMMDDYYtoYYYYMMDD(
              String(discountDueDate).padStart(6, "0")
            ),
      invoiceNo,
      extendedInvoiceDate: convertMMDDYYtoYYYYMMDD(
        String(invoiceDate).padStart(6, "0")
      ),
      extendedDueDate: convertMMDDYYtoYYYYMMDD(
        String(dueDate).padStart(6, "0")
      ),
      prepaidCheckdate8: (!prepaidCheckdate || prepaidCheckdate === 0) ? 0 : convertMMDDYYtoYYYYMMDD(
        String(prepaidCheckdate).padStart(6, "0")
      ),
      status:
