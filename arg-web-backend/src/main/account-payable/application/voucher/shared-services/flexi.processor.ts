...
import { tableType } from "@src/shared/constants/voucher-type.enum";
...
export class FlexiProcessor {
  ...
  private async processGroup(group: any) {
    ...
    // Validate header first to get foundVendor
    const headerResult = await this.validateHeader(headerDTO);
    
    // Check if any detail item has a discount (amount or percentage)
    const anyDetailHasDiscount = group.details.some((detail) => {
      const discountAmount = Number(detail[csvFlexiDetailMapping.discountAmount]) || 0;
      const discountPercentage = Number(detail[csvFlexiDetailMapping.discountPercentage]) || 0;
      return discountAmount > 0 || discountPercentage > 0;
    });
    
    // Map details with vendor information for discount logic (now async)
    const detailDTOs = await Promise.all(
      group.details.map((detail) => 
        this.mapDetail(detail, headerResult.foundVendor, anyDetailHasDiscount)
      )
    );
  ...
  private async validateHeader(headerDTO: HeaderDto) {
    ...
    return {
      ...
      foundVendor: result.foundVendor, // Expose foundVendor for detail mapping
  ...
  private async mapDetail(detailRow: Record<string, any>, foundVendor?: any, anyDetailHasDiscount: boolean = false): Promise<DetailDto> {
    // Get discount values from CSV
    const csvDiscountAmount = Number(detailRow[csvFlexiDetailMapping.discountAmount]) || 0;
    const csvDiscountPercentage = Number(detailRow[csvFlexiDetailMapping.discountPercentage]) || 0;
    
    // Determine final discount values
    let finalDiscountAmount = csvDiscountAmount;
    let finalDiscountPercentage = csvDiscountPercentage;
    
    // Only apply vendor discount if NONE of the detail items have a discount
    // If any detail item has a discount, skip vendor discount for all items
    if (!anyDetailHasDiscount && !csvDiscountAmount && !csvDiscountPercentage && foundVendor?.vendorApTermsCode) {
      try {
        const generalSystemRecord = await this.generalSystemService.getGeneralSystemRecord(
          tableType.APTERM,
          foundVendor.vendorApTermsCode.toString()
        );
        
        if (generalSystemRecord && generalSystemRecord.discount) {
          // Convert discount from decimal to percentage (e.g., 0.02 -> 2)
          finalDiscountPercentage = generalSystemRecord.discount * 100;
          this.logger.log(
            `Using vendor discount: ${finalDiscountPercentage}% (from APTERM ${foundVendor.vendorApTermsCode}) for vendor ${foundVendor.vendorNo}`
          );
        }
      } catch (error) {
        this.logger.warn(
          `Failed to fetch discount from General System for APTERM code ${foundVendor.vendorApTermsCode}: ${(error as Error).message}`
        );
      }
    }
