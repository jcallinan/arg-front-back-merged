...
import { convertDateFormat, convertMmddyyToFormats, convertMMDDYYtoYYYYMMDD, DATE_FORMATS } from "@src/shared/utils/format-date";
...
export class CheckInquiryRepository implements CheckInquiryInterface {
  ...
  async getLastPaymentInfo(data: getLastPaymentInfo): Promise<lastPaymentInfo> {
    ...
    if (startDate) {
      updatedStartdate = convertMMDDYYtoYYYYMMDD(startDate)
      this.logger.log(`Updated StartDate: ${updatedStartdate}`)
    }
    ...
    const lastPaymentInfo = await this.checkInquiryModel.findOne({
      ...
      where: {
        ...
        ...(checkNo && { checkNo }),
        ...(invoiceNo && { invoiceNo }),
        ...(startDate && { paidDate: { [Op.gte]: updatedStartdate } }),
      },
      include: [
        {
    ...
    return {
      ...
      lastPaidDate:
        lastPaymentInfo?.lastPaidDate && lastPaymentInfo.lastPaidDate !== 0
          ? convertDateFormat(lastPaymentInfo.lastPaidDate, DATE_FORMATS.YYMMDD, DATE_FORMATS.MMDDYY)
  ...
  async getPaymentHistory(data: getPaymentHistory): Promise<{
    ...
    try {
      ...
      // Convert Date MMDDYY to YYYYMMDD
      let updatedStartdate;
      if (startDate) {
        updatedStartdate = convertMMDDYYtoYYYYMMDD(startDate)
        this.logger.log(`Updated StartDate: ${updatedStartdate}`)
      ...
      const { rows, count } = await this.checkInquiryModel.findAndCountAll({
        attributes: [
          ...
          [
            literal(`
              CASE 
                WHEN "bankGLNumber"."${(this.checkInquiryHistoryModel as any).getAttributes().checkDate.field
              }" IS NOT NULL 
        ...
        where: {
          ...
          ...(invoiceNo && { invoiceNo }),
          ...(startDate && {
            paidDate: { [Op.gte]: updatedStartdate },
          }),
        },
        ...
        include: [
          {
            ...
            on: {
              [Op.and]: [
                where(
                  col(
                    `CheckInquiryModel.${(this.checkInquiryModel as any).getAttributes().bankGLNo.field
                    }`
                  ),
                  Op.eq,
                  col(
                    `bankGLNumber.${(this.checkInquiryHistoryModel as any).getAttributes().bankGLNo.field
                    }`
                  )
                ),
                where(
                  col(
                    `CheckInquiryModel.${(this.checkInquiryModel as any).getAttributes().checkNo.field
                    }`
                  ),
                  Op.eq,
                  col(
                    `bankGLNumber.${(this.checkInquiryHistoryModel as any).getAttributes().checkNo.field
                    }`
                  )
                ),
              ],
      ...
      const formattedRows = rows.map((row: any) => {
        ...
        plain.paidOnDate =
          plain.paidDate && plain.paidDate !== 0
            ? convertDateFormat(plain.paidDate, DATE_FORMATS.YYYYMMDD, DATE_FORMATS.MM_DD_YY)
            : plain.paidDate;

        plain.lastPaidDate =
          plain.lastPaidDate8 && plain.lastPaidDate8 !== 0
            ? convertDateFormat(plain.lastPaidDate8, DATE_FORMATS.YYYYMMDD, DATE_FORMATS.MM_DD_YY)
            : plain.lastPaidDate8;
