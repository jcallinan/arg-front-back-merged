...
import { convertDateFormat, DATE_FORMATS, formatMMDDYYDate } from "@src/shared/utils/format-date";
  ...
  implements VoucherMaintenanceInterface
{
      ...
      const entities = rows.map((row) => {
        ...
        return new VoucherMaintenance({
          ...
          lastPaidDate8: isFromHistory
            ? (row.lastPaidDateYymmdd && row.lastPaidDateYymmdd !== 0 ? convertDateFormat(row.lastPaidDateYymmdd,DATE_FORMATS.YYMMDD, DATE_FORMATS.MMDDYY) : row.lastPaidDateYymmdd)
            : row.lastPaidDate8,
          ...
          paidOnYymmdd: isFromHistory ? (row.paidOnYymmdd && row.paidOnYymmdd !== 0 ? convertDateFormat(row.paidOnYymmdd,DATE_FORMATS.YYMMDD, DATE_FORMATS.MMDDYY) : row.paidOnYymmdd) : undefined,
  ...
  private buildWhereClause(
    ...
    if (invoiceDate) {
      ...
      const buildDateConversionSQL = (dateCol: string) => {
        const d = `RIGHT(DIGITS(${dateCol}), 6)`;
      ...
      if (voucherType === VoucherType.PAID) {
        ...
        where[Op.and as any] = [
          ...(where[Op.and as any] || []),
          literal(`${buildDateConversionSQL(dateCol)} >= ${invoiceDateYYYYMMDD}`)
        ];
      } else {
    ...
    this.logger.log(`UNPAID where: ${JSON.stringify(where)}`);
    return where;
  }
  ...
  private buildOrderClause(
    ...
    this.logger.log(`UNPAID order: ${JSON.stringify(order)}`);
    return order;
  ...
  async transferVoucher(
    ...
    try {
      ...
      if (voucherType === VoucherType.UNPAID) {
            ...
            errorResponse(ERROR_CONSTANTS.VALIDATION_ERROR, [
              {
                field: "voucher",
                code: ERROR_CONSTANTS.VALIDATION_ERROR.code,
                message: ERROR_MESSAGES.VOUCHER_EXITS_IN_NORMAL_VOID,
              },
            ]),
            HttpStatus.BAD_REQUEST
          );
        ...
        if (checkInquiryHistory?.code === AMCODE.VOIDED){
            throw new HttpException(
              errorResponse(ERROR_CONSTANTS.VALIDATION_ERROR, [
                {
                  field: "voucher",
